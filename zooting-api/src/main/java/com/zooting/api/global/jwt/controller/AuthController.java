...
import com.zooting.api.global.jwt.dto.AuthTokenReq;
...
@Tag(name = "Auth", description = "토큰 발급 관련 컨트롤러")
public class AuthController {
    private final JwtService jwtService;
    private final CustomUserDetailsService userDetailsService;
    @Operation(summary = "액세스 토큰 재발급 요청하기", description = "액세스 토큰 없거나 만료됐으면 재발급 요청하기")
    @PostMapping("/refresh")
    public ResponseEntity<?> sendRefreshRequest(@Valid @RequestBody AuthTokenReq authTokenReq){
        String refreshToken = authTokenReq.refreshToken();
