      ...
      <ul role="list" class="friend-list">
        ...
        <li v-for="(item, index) in friendList" :key="index" :class="[item.nickname === openedMenuNickname ? 'bg-gray-50': '', 'friend-list__item']">
          ...
          <div class="menu" v-show="isOpenMenu && openedMenuNickname === item.nickname">
            <div class="py-2">
              <div v-if="item.isOnline" class="block px-4 py-2 text-sm text-gray-700 cursor-pointer hover:underline" @click="meetingRequestFriend(item.nickname)">미팅 신청</div>
              <p v-else class="block px-4 py-2 text-sm text-gray-300">미팅 신청</p>
            </div>
            <div class="py-2 text-sm text-gray-700 hover:underline">
              <RouterLink :to="getProfileLink(item.nickname)" class="block px-4 py-2">프로필 보기</RouterLink>
            </div>
            <div class="py-2">
              <div class="block px-4 py-2 text-sm text-red-700 cursor-pointer hover:underline" @click="friendDelete(item.nickname)">친구 삭제</div>
            </div>
          </div>
        </li>
      </ul>
...
.friend-list__item {
  @apply relative flex justify-between px-6 py-4;
}
...
.menu-button {
  @apply w-6 h-6 text-gray-800 cursor-pointer;
}
.menu {
  @apply absolute z-10 bg-gray-50 flex flex-row top-0 left-0 h-full w-[80%] justify-center items-center;
}
.video-chat-alert {
  @apply absolute z-20;
  width: 428px;
